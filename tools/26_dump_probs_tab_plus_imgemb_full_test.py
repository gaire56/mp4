...
def find_emb(emb_split_dir, dataset_tag, base, emb_kind):
...
def main():
    ...
    emb_split = os.path.join(args.emb_dir, "test")

    ys, ps = [], []

    for sh in shards:
        Xtab = np.load(sh["X"], mmap_mode="r")
        ...
        emb_path = find_emb(emb_split, args.dataset_tag, sh["base"], args.emb_kind)
