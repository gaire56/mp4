...
Table~\ref{tab:model_comparison} reports final performance on the full EMBER2024 PE-only test split for the Tab-only and Tab+ImgEmb models.
For the final Tab+ImgEmb model, ROC-AUC is 0.99795 and PR-AUC is 0.99805.
At $\tau=0.50$, the model reaches Accuracy = 0.9795, Precision = 0.9875, Recall = 0.9713, and F1 = 0.9793.
At the best-F1 operating point ($\tau=0.353$), recall improves to 0.9774 and F1 rises to 0.9801.
The table also reports thresholds corresponding to approximate false-positive-rate regimes of 1\%, 0.5\%, and 0.1\% ($\tau=0.593$, $\tau=0.823$, and $\tau=0.979$ for the fusion model).
As expected, stricter thresholds increase precision but reduce recall, so threshold selection should be treated as a deployment policy choice rather than a single universal operating point.

\begin{table}[t]
\centering
\caption{Final malware detection results on the full EMBER2024 PE-only test split at selected operating points.}
...
Table~\ref{tab:operational_rates} summarizes operating-point behavior using rates rather than counts.
As the threshold increases, FPR decreases, but TPR also decreases.
At $\tau=0.353$, the model achieves TPR = 0.9774 at FPR = 0.0171, while at $\tau=0.500$ it reaches TPR = 0.9713 at FPR = 0.0123.
At stricter thresholds such as $\tau=0.823$ and $\tau=0.979$, FPR falls to about 0.5\% and 0.1\%, respectively, but TPR also decreases.
This makes threshold selection a practical choice between missed malware and analyst workload.

\begin{table}[t]
\centering
\caption{Operational rates for the final Tab+ImgEmb model on the full EMBER2024 PE-only test set at selected thresholds.}
