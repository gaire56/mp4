...
Feature normalization statistics are computed on the training split and reused during inference and explanation generation so that model scores and explanation outputs remain aligned with the trained pipeline.
This is especially important in analyst-facing settings, where inconsistent preprocessing can distort both probabilities and explanation evidence.
...
To make malware decisions reviewable, explanations are summarized in compact tabular form aligned with the implemented late-fusion architecture.
...
To fit publication constraints, explanation outputs are summarized as compact tables rather than multiple heatmaps.
