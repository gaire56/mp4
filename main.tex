...
To improve decision transparency, the pipeline integrates SHAP for global and per-file explanation of the final LightGBM late-fusion classifier and Grad-CAM for structural evidence from the CNN-based section model, summarized compactly to fit publication constraints.
...
For the implemented pipeline, SHAP is used to explain the final LightGBM late-fusion classifier, while Grad-CAM provides complementary structural evidence for the CNN-based section model~\cite{LL2017,SCDVPB2017}.
...
For the final Tab+ImgEmb LightGBM classifier, we use SHAP to generate explanation evidence over the fused feature vector, including global feature rankings and SHAP-based per-sample feature attribution capability~\cite{LL2017}.
...
The Tab+ImgEmb late-fusion model combines two sources of evidence: the EMBERv3 tabular feature vector and an exported image embedding derived from a CNN-based section model operating on section-structure representations.
...
We evaluated two malware detection models: a Tab-only LightGBM baseline and a Tab+ImgEmb LightGBM late-fusion model that augments the same tabular evidence with an image embedding exported from a CNN-based section model.
...
SHAP explained the final LightGBM late-fusion classifier, while Grad-CAM provided complementary structural evidence from the CNN-based section model.
