...
Table~\ref{tab:model_comparison} reports both threshold-independent and threshold-specific performance.
For the final Tab+ImgEmb model, ROC-AUC is 0.99795 and PR-AUC is 0.99805, indicating strong separability and high precision across the recall range.
At the standard threshold ($\tau=0.50$), the fusion model reaches Accuracy = 0.9795, Precision = 0.9875, Recall = 0.9713, and F1 = 0.9793.
At the best-F1 operating point ($\tau=0.353$), recall improves to 0.9774 and F1 increases to 0.9801, with a moderate increase in false positives.
The table also reports thresholds for approximate false-positive-rate regimes of 1\%, 0.5\%, and 0.1\%, which for the fusion model occur at $\tau=0.593$, $\tau=0.823$, and $\tau=0.979$.
As expected, stricter thresholds increase precision but reduce recall.
For example, precision rises from 0.9875 at $\tau=0.50$ to 0.9989 near 0.1\% FPR, while recall drops from 0.9713 to 0.8901.
These results show that threshold selection should be treated as an explicit deployment policy choice rather than a single universal operating point.

\begin{table}[t]
\centering
\caption{Final malware detection results on the full EMBER2024 test set. ROC-AUC and PR-AUC are threshold-independent; the remaining metrics are reported at selected operating points.}
...
\hline
\textbf{Model / Operating Point} & \boldmath$\tau$ & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{ACC} & \textbf{Prec.} & \textbf{Recall} & \textbf{F1} \\
\hline
Tab-only LGBM (standard)                    & 0.500 & 0.997938 & 0.99813 & 0.979406 & 0.9877 & 0.9709 & 0.9792 \\
Tab-only LGBM ($\mathrm{FPR}\approx 1\%$)   & 0.593 & 0.997938 & 0.99813 & 0.978404 & 0.9898 & 0.9668 & 0.9782 \\
Tab-only LGBM ($\mathrm{FPR}\approx 0.5\%$) & 0.823 & 0.997938 & 0.99813 & 0.972381 & 0.9948 & 0.9498 & 0.9717 \\
Tab-only LGBM ($\mathrm{FPR}\approx 0.1\%$) & 0.979 & 0.997938 & 0.99813 & 0.944525 & 0.9989 & 0.8901 & 0.9413 \\
Tab+ImgEmb LGBM (standard)                  & 0.500 & 0.997949 & 0.99805 & 0.979481 & 0.9875 & 0.9713 & 0.9793 \\
Tab+ImgEmb LGBM (best-F1)                   & 0.353 & 0.997949 & 0.99805 & 0.980175 & 0.9829 & 0.9774 & 0.9801 \\
Tab+ImgEmb LGBM ($\mathrm{FPR}\approx 1\%$)   & 0.593 & 0.997949 & 0.99805 & 0.978404 & 0.9898 & 0.9668 & 0.9782 \\
Tab+ImgEmb LGBM ($\mathrm{FPR}\approx 0.5\%$) & 0.823 & 0.997949 & 0.99805 & 0.972381 & 0.9948 & 0.9498 & 0.9717 \\
Tab+ImgEmb LGBM ($\mathrm{FPR}\approx 0.1\%$) & 0.979 & 0.997949 & 0.99805 & 0.944525 & 0.9989 & 0.8901 & 0.9413 \\
\hline
\end{tabular}%
}
\vspace{-5mm}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.34\textwidth]{curves/ROC_tab_plus_imgemb_full_test.png}
...
Table~\ref{tab:operational_rates} summarizes operating-point behavior using rates rather than absolute counts.
As the threshold increases, the false-positive rate decreases, but the false-negative rate increases.
At the best-F1 operating point ($\tau=0.353$), the model achieves TPR = 0.9774 at FPR = 0.0171.
At the standard threshold ($\tau=0.50$), TPR is 0.9713 and FPR is 0.0123.
At stricter operating points, such as $\tau=0.823$ and $\tau=0.979$, the FPR falls to approximately 0.5\% and 0.1\%, respectively, but TPR also decreases to 0.9498 and 0.8901.
This makes threshold selection a practical deployment choice between missed malware and analyst workload.
